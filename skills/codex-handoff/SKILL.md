---
name: handoff
description: Create a durable project handoff before a clear/new session, rotation, or a stage boundary.
---
...
2. Run `agent-context handoff --harness codex --mode rotate` with concise `--objective`, `--next-action`, and optional `--note` values.
...
```
agent-context acknowledge --harness codex --status accepted|rejected|needs-clarification [--note "..."]
```
